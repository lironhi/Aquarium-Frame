...
public class Memento implements ActionListener {
	...
	public Memento(AquaPanel panel) {
		...
		if (panel.setAnimals.size() < 1 && panel.setPlants.size() < 1) {
			...
			int i = 0;
			for (SeaCreature s : panel.setAnimals) {
				JLabel type3 = new JLabel(i + 1 + ") " + s.getAnimalName() + " size: " + s.getSize() + ", color:"
						+ s.getColor() + ", verSpeed: " + s.getVerSpeed() + ", horSpeed: " + s.getHorSpeed() + ",X: "
						+ s.getX() + ", Y: " + s.getY());
			...
			int j = 0;
			for (SeaPlant s : panel.setPlants) {
				JLabel type3 = new JLabel(i + 1 + ") " + s.getName() + " size: " + s.getSize() + ", color: Green , x: "
						+ s.getX() + " , y: " + s.getY());
				MemoOption.add(type3);
				arr2[j] = (Immobile) s;
				j++;
		...
		if ((Fcounter == 0 && Pcounter == 0)) {
			...
			if (this.Fcounter != 0) {
				for (Swimmable s : SwimArr) {
					JLabel type3 = new JLabel(i + 1 + ") " + s.getAnimalName() + " size: " + s.getSize() + ", color:"
							+ s.getColor() + ", verSpeed: " + s.getVerSpeed() + ", horSpeed: " + s.getHorSpeed()
							+ ",X: " + s.getX() + ", Y: " + s.getY());
					MemoOption.add(type3);
					i++;
				}
			...
			if (this.Pcounter != 0) {
				for (Immobile im : PlantArr) {
					JLabel type3 = new JLabel(i + 1 + ") " + im.getName() + " size: " + im.getSize()
							+ ", color: Green , x: " + im.getX() + " , y: " + im.getY());
					MemoOption.add(type3);
					i++;
				}
			}
