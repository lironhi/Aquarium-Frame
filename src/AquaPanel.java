...
public class AquaPanel<animals> extends JPanel implements ActionListener
{
   ...
   private String[] names = {"Add Animal","Add Plant", "Sleep","Wake up","Reset","Food","Info","Duplicate Animal","Exit"};
   ...
   synchronized public void food()
   {
	   if(!animals.isEmpty())
	   {
		   if (sglt!=null && isFood == true)
			{
				JOptionPane.showMessageDialog(this, "The worm is already in the aquarium! ","Error",JOptionPane.ERROR_MESSAGE);
			}
		   else{
   ...
   public void duplic() {
	   if (setAnimals.size()>=1)
		...
		} else JOptionPane.showMessageDialog(this, "no type","Error!",JOptionPane.ERROR_MESSAGE);
   }
   public void destroy()
   {  	        
      System.exit(0);
